...
Layered test harness for the seance renderer. Phase A: Layer 1 (pure logic / VT
behavior) + Layer 4 (frame-assembly snapshots). Future phases add font raster
(L2), shaper (L3), headless GPU (L5), PNG goldens (L6), property/fuzz (L8),
benches (L9).
...
- **`tests/layer1_logic.rs`** — plain `#[test]`. Asserts observable VT behavior
...
1. Read the diff. The grid box and the `cells:` block show what the renderer
...
Phase B expands the catalog to include ligatures, emoji ZWJ, scrollback, resize
sequences, and Kitty graphics streams.
